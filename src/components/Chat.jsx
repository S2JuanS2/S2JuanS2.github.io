...
const Chat = () => {
  ...
  const [userName, setUserName] = useState(localStorage.getItem("userName") || "Anonimo");
  ...
  useEffect(() => {
    ...
    socket.current.onmessage = (event) => {
      ...
      setMessages((prevMessages) => {
        if (!prevMessages.includes(receivedMessage)) {
      ...
      <div className='border-2 rounded-sm pt-4 border-black bg-gray-800 mb-3'>
        <div className="overflow-auto h-64 mb-4">
          {messages.map((msg, index) => {
            const [msgUser, msgText] = msg.split(': ');
            return (
              <div key={index} className={`pt-1 pl-2 pr-2 mb-2 bg-gray-100 rounded-lg mr-4 ml-4 h-7 w-max ${msgUser === userName ? 'bg-green-200' : ''}`}>
                <span className="font-bold">{msgUser}:</span> {msgText}
              </div>
            );
          })}
        </div>
